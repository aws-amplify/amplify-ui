...
if [ "$PKG_MANAGER" == 'yarn' ]; then
    ...
    if [[ "$FRAMEWORK" == "react-native" ]]; then
        DEPENDENCIES="$TAGGED_UI_FRAMEWORK @aws-amplify/react-native aws-amplify react-native-safe-area-context @react-native-community/netinfo @react-native-async-storage/async-storage react-native-get-random-values react-native-url-polyfill ts-node @types/node"
