...
if [ "$PKG_MANAGER" == 'yarn' ]; then
    ...
    if [[ "$FRAMEWORK" == "react-native" ]]; then
        ...
        DEPENDENCIES="$TAGGED_UI_FRAMEWORK @aws-amplify/react-native aws-amplify @react-native-community/netinfo @react-native-async-storage/async-storage@^2.2.0 react-native-get-random-values@1.11.0 react-native-url-polyfill"
