...
**_Note_**: When you use SearchField in controlled way, it is your responsibility to set up `onClear` other than `onChange` since the input value is under your control.
...
<Example>
  <SearchField label="Globally styled search field" className="globally-styled-searchfield" />
...
<Example>
  <SearchField className="custom-searchfield-class" label="Square password field" />
