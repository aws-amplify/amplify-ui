...
import {
  ...
  UncontrolledAutocompleteExample,
...
To enable autocomplete, simply provide a `suggestions` prop

#### Uncontrolled autocomplete
<Example>
  <UncontrolledAutocompleteExample />
  <ExampleCode>
    ```tsx file=./examples/UncontrolledAutocompleteExample.tsx
...
<Example>
  <ControlledAutocompleteExample />
  <ExampleCode>
    ```tsx file=./examples/ControlledAutocompleteExample.tsx

    ```
  </ExampleCode>
</Example>

**Note**: When you use autocomplete in a controlled way, you are also responbile to setup `onClear` and `onSuggestionSelect` event listeners
other than the `onChange` since the input value is under your control.
