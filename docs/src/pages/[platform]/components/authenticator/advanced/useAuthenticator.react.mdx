...
In advanced use cases where usage of the [`useAuthenticator` hook](/components/authenticator#useauthenticator-hook) 
outside the scope of the [`Authenticator`](/components/authenticator) is needed, wrap your application inside 
a `Authenticator.Provider`. The `Authenticator.Provider` guarantees that the [useAuthenticator hook](/components/authenticator#useauthenticator-hook) 
is available throughout your application. You can see an example of this pattern in the 
[Protected Routes Guide.](/guides/auth-protected)
