...
Message can be customized via props as a basic component, or it can be used as a [composable component](#composable) which provides more control over layout and functionality.
...
`Message.Dismiss` can be customized to hide the icon by setting the prop `hasIcon={false}`. It can also accept custom button text and other props that our [Button](./button) primitive accepts.
