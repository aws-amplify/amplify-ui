...
- `leftArrowLabel`: Set the `aria-label` for the left arrow button `<` (defaults to `Go to previous page`)

- `rightArrowLabel`: Set the `aria-label` for the right arrow button `>` (defaults to `Go to next page`)
