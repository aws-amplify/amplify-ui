...
By default, the root node of the Pagination component has a role of `navigation`. Accessible labels are provided for the pagination buttons either via the `aria-label` attribute or by using the [VisuallyHidden](/react/components/visuallyhidden) primitive.
