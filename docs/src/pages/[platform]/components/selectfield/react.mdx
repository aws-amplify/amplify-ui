...
The `multiple` attribute specifies that multiple options can be selected at once. If the `multiple` attribute is present, the default value for the `size` attribute is 4. Use the `isMultiple` prop to set the state for the SelectField. 
...
The `size` attribute sets the number of visible options in a drop-down list. Use the `selectSize` prop to set the `size` attribute value. If the value of the `size` attribute is greater than 1, but lower than the total number of options in the list, the browser will add a scroll bar to indicate that there are more options to view.

Default value is 1, but if the `multiple` attribute is present, the default value is 4.
