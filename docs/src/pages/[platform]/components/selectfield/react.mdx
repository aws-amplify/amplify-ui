...
The `size` attribute sets the number of visible options in a drop-down list. Use the `selectSize` prop to set the `size` attribute value. If the value of the `selectSize` prop is greater than 1, but lower than the total number of options in the list, the browser will add a scroll bar to indicate that there are more options to view.
