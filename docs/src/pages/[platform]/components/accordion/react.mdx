...
To only allow one item to be open at a time, add the `isExclusive` prop.
...
The default behavior allows opening multiple items at once.
...
To allow the accordion to have all items collapsed, set the `isCollapsible` prop.

**Note**: The `isCollapsible` prop only applies to the `single` Accordion, and it is set to `false` (not collapsible) by default.
