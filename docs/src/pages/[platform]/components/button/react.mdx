...
import { ButtonLinkExample, ButtonThemeExample, IconButtonExample } from './examples';
...
To use the Button as a link, pass a URL to the `href` prop. This will render an anchor element `<a>` with Button styling.  
...
```tsx file=./examples/ButtonLinkExample.tsx
