...
Import the `Menu` and `MenuItem` components. Use the `MenuItem` component to configure menu options.
...
To control the alignment of the menu with the trigger button, use the `menuAlign` prop. Available options are `start` (default), `center`, and `end`.
