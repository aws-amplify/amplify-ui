...
import {
  ...
  NoResultsFoundCollectionExample,
...
To handle the case when no results are found from the search, you can pass a custom `ReactNode` (includes `string`) to the `noResultsFound` prop. By default, no search results renders the text "No results found".

<Example>
  <NoResultsFoundCollectionExample />
  <ExampleCode>

```tsx file=./examples/NoResultsFoundCollectionExample.tsx
