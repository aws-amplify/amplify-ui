...
<Example>
  <ComposableDropZoneExample />
  <ExampleCode>
  ```tsx file=./examples/ComposableDropZoneExample.tsx
  ```
  </ExampleCode>
</Example>
...
You can build your own DropZone component with the `useDropZone` hook. `useDropZone` will return the event handlers needed to add onto an element as well as state: `isDragActive`, `isDragAccept`, `isDragReject`.

<Example>
  <UseDropZoneExample />
  <ExampleCode>
  ```tsx file=./examples/UseDropZoneExample.tsx
  ```
  </ExampleCode>
</Example>
