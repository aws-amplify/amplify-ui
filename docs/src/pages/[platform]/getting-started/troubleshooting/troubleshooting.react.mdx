...
Amplify UI components are interactive and designed to work on the client side. To use them inside of Server Components, you must wrap them in a Client Component with [the `"use client"` directive](https://react.dev/reference/rsc/use-client). For more information, see the Next.js [documentation](https://nextjs.org/docs/app/building-your-application/rendering/composition-patterns#using-third-party-packages-and-providers) on how you could achieve this.
