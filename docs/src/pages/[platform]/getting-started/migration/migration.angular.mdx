...
## 2.x to 3.x
### Installation
Install the 3.x version of the `@aws-amplify/ui-angular` library.

<Tabs>
<TabItem title="npm">

```shell
npm install aws-amplify @aws-amplify/ui-angular@3.x
```

</TabItem>
<TabItem title="yarn">
```shell
yarn add aws-amplify @aws-amplify/ui-angular@3.x
```
...
```
#### 1. Update to the latest version of the `aws-amplify` library.

Update to the latest version of the `aws-amplify` library. You will need at least version `^5`. 

<Tabs>
<TabItem title="npm">

```shell
npm install aws-amplify@5.x
```

</TabItem>
<TabItem title="yarn">
```shell
yarn add aws-amplify@5.x
```

</TabItem>
</Tabs>

#### 2. Update to Angular 12 or later

If your application is on Angular 11 or before, please migrate to Angular 12+ as per [official guide](https://update.angular.io).
...
#### 3. Migrate Angular compiler to IVY

We dropped support for legacy View engine, and migrated to IVY compiler. lease migrate to Angular 12+, and make sure you did not disable ivy in your `tsconfig.json`:
...
#### 4. Legacy i18n translation keys removed
...
#### 5. Update the handleSignUp function override call
...
The 2.x version of the `Authenticator` has a different format for the `formFields` prop. It also no long accepts
