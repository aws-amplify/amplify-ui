...
#### 3. `@aws-amplify/ui-react@5.x` updates translation key for korean language from `kr` to `ko`.

Use `ko` key for korean translations. Update any existing usages.
...
#### 4. `@aws-amplify/ui-react@5.x` remove the `to` prop on `Link` component and instead have it extended from the underlying rendered third-party `Link` if it contains a `to` prop.

For example, using React Router Link with Amplify Link.
...
#### 5. `@aws-amplify/ui-react@5.x` updates component types to include the underlying rendered HTML element's attributes and strictly types the `View` component.
