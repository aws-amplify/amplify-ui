...
#### 1. Expander is now Accordion
The Expander component has been replaced by the Accordion component in Amplify UI v6. Here is a mapping between the two components:

**Expander -> Accordion**
The main Expander component maps to the Accordion.Container component.
...
**ExpanderItem -> Accordion.Item**
The `ExpanderItem` component maps to the `Accordion.Item` component.
...
The ExpanderItem's `title` prop maps to placing content inside the `<Accordion.Trigger>` component.

The ExpanderItem's `value` prop maps to the `value` prop on `<Accordion.Item>`.

The children of `<ExpanderItem>` map to inside `<Accordion.Content>`.
...
* The Accordion cannot be fully collapsed by default, while the Expander could be collapsed. To make the Accordion always have 1 item open, use the isAlwaysOpen prop.
...
<Tabs>
<TabItem title="npm">
<TerminalCommand command="npm install aws-amplify @aws-amplify/ui-react-liveness@2.x" />
</TabItem>
<TabItem title="yarn">
<TerminalCommand command="yarn add aws-amplify @aws-amplify/ui-react-liveness@2.x" />
</TabItem>
</Tabs>
