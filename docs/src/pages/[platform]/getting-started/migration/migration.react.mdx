...
<Alert variation="info" heading="Note">
  If you are NOT using `dir="rtl"` in your application, feel free to skip this
  item.
</Alert>
...
<Alert variation="info" heading="Note">
  The TextAreaField component does apply 1 style prop directly on the `textarea`
  element: `resize`. We felt this one makes sense to apply direclty on the the
  `textarea` element and not the wrapper element.
</Alert>
