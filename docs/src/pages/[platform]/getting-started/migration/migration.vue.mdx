...
sense of the changes please check out the [Headers and Footers](/components/authenticator#headers--footers) section.

### Form Fields

The latest version of the `Authenticator` has a different format for the `formFields` prop. It also no long accepts
`inputProps` nor `hint`. Instead it's recommended that you use the [Headers and Footers Slots](/components/authenticator#headers--footers) or use the
[Sign Up Fields custimization](/components/authenticator#sign-up-fields). For more information on form field customizations
please see the [Form Field Customization](/components/authenticator#form-field-customization) section.
