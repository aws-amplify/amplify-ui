...
To use Connected componnets in app directory, make sure you re-export `Authenticator` as a Client Component by placing "use client" at the top of the file and set up with `Amplify.configure`, for example:
...
Then, import `Authenticator` from the Client Component module `Authenticator.jsx` and use it inside `app/page.jsx`.
...
To use Connected componnets in page directory, make sure you set up `Amplify.configure` in `src/pages/_app.jsx`
    <ExampleCode>
