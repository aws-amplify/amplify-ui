...
All the styling is handled by plain CSS and theme-able with CSS variables. However we do provide some extra styling utilities in React-like responsive style props, [ThemeProvider and useTheme hook](/react/theming), and [useBreakpointValue](/react/theming/responsive#usebreakpointvalue) for responsive values.
...
We want to share as much as possible between platforms (like themes and schemas), and respect each platform identity and rules when possible (e.g. following name conventions and idioms). React components should feel like React components.
