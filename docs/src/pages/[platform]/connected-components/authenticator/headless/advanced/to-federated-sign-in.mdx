import { ResponsiveTableCell } from '@/components/ResponsiveTable';
import { TableRow, TableCell } from '@aws-amplify/ui-react';

<TableRow>
  <ResponsiveTableCell label="Name">`toFederatedSignIn`</ResponsiveTableCell>
  <ResponsiveTableCell label="Description">
    Transitions to `provider`'s federated sign in page. Supported `provider`
    values can be found
    [here](https://github.com/aws-amplify/amplify-ui/blob/96830f6a34a417aa9bc6329c839679bd10da84f0/packages/ui/src/helpers/auth.ts#L104-L109).
  </ResponsiveTableCell>
  <ResponsiveTableCell label="Type">
    `(provider: string) => void`
  </ResponsiveTableCell>
</TableRow>
