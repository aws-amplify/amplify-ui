...
First, add these entries to your existing `lib/l10n/amplify_en.arb` file: 
...
```json
  ...
  "authenticatorCognitoErrorUserNotConfirmed": "Your account is not confirmed. Please check your email for a confirmation link.",
  "@authenticatorCognitoErrorUserNotConfirmed": {
    "description": "Error message when user account is not confirmed"
  },
  "authenticatorCognitoErrorUnknown": "An unknown error occurred. Please try again.",
  "@authenticatorCognitoErrorUnknown": {
    "description": "Fallback error message for unknown errors"
  },
```

Note, that a full list of error types that can be translated can be found 
[here](https://github.com/aws-amplify/amplify-flutter/blob/main/packages/authenticator/amplify_authenticator/lib/src/models/authenticator_exception.dart#L12). 
...
class LocalizedMessageResolver extends MessageResolver {
  ...
  String error(BuildContext context, AuthenticatorException exception) {
    ...
    if (exception is CognitoAuthenticatorException) {
      ...
      switch (arbKey) {
        case 'authenticatorCognitoErrorUserNotFound':
          return localizations.authenticatorCognitoErrorUserNotFound;
        case 'authenticatorCognitoErrorUserNotConfirmed':
          return localizations.authenticatorCognitoErrorUserNotConfirmed;
    ...
    return exception.message.isNotEmpty 
        ? exception.message 
        : localizations.authenticatorCognitoErrorUnknown;
  }
}
```
...
```dart
const stringResolver = AuthStringResolver(
  buttons: LocalizedButtonResolver(),  // from previous example
