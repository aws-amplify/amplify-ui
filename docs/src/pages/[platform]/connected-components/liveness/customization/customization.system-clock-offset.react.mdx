...
The `systemClockOffset` can optionally be passed to the `FaceLivenessDetectorCore` config, where it will be used added an offset in milliseconds to the AWS server time. This can be useful when the user's device time is inaccurate or when the user is frequently switching time zones. An example of calculating the offset and passing it to the `FaceLivenessDetectorCore` is shown below:
