...
2. Ensure that the session ID returned from the backend when creating the session is the same as the one started use by the app. Note: session ID's can only be used once. You must create a new session to allow users to retry.
