...
import {
  ...
  ComposedDeleteView,
  ComposedLocationsView,
  ComposedLocationDetailView,
  ComposedUploadView,
  CustomCreateFolderView,
  CustomCopyView,
  CustomDeleteView,
  CustomLocationsView,
  CustomLocationDetailView,
  CustomUploadView,
  DisplayText,
  Theming,
  Icons,
...
If you wanted to move around some of the components in any of the views without wanting to rebuild the entire view yourself, you can use the composable components of the view(s) you wish to customize. You will first need to use the `useView('[view name]')` hook and pass its return to the `<StorageBrowser.[view name].Provider>` and then the children of the provider can be the composed sub-components. 

Below you wll find examples of how to compose each view. See [View reference](#view-reference) for all the composable components each view has. 

<Tabs.Container defaultValue="Locations">
  <Tabs.List>
...
The `createStorageBrowser` also returns a `useView()` hook. This hook lets you use the Storage Browser's internal state and handlers to build your own UI on top the Storage Browser functionality.
...
Below you will find examples of building custom UI for each view:
...
Below you will find the interfaces that the `useView()` hook returns for a given view as well as the composable sub-components on the view.
