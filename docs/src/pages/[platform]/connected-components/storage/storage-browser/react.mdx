...
<ExampleCode>
```bash
npx copy-serviceworker public
```
</ExampleCode>
</Example>

This copies `download-sw.js` into your `public/` directory, making it available at the root of your application.
...
{
  "scripts": {
    "prebuild": "copy-serviceworker public"
  }
}
```
</ExampleCode>
...
1. Registers the service worker at `/download-sw.js`
...
Control the number of items displayed per page in file and folder listings using the `pageSize` prop. Defaults to 100 items per page.
...
The `pageSize` applies to the Locations, Location Detail, and Upload views. The value must be a positive integer; invalid values fall back to the default with a console warning.
...
<Message colorTheme="warning" variation="filled" heading="Folder deletion requirement">
Folder deletion functionality requires aws-amplify package version 6.16.0 or later.
</Message>
...
* `<StorageBrowser.DeleteView.ConfirmationModal />`
