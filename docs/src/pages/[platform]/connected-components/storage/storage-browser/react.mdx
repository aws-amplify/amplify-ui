...
The `createStorageBrowser` function allows you to customize the Storage Browser settings by passing an `options` configuration object. The `options` accepts a `validateFile` prop that can be used to replace the default file validation performed when selecting files. The `validateFile` function takes in a single file and return `true` if the file is considered valid. 
