...
The `createStorageBrowser` function allows for further overrides to Storage Browser settings to be specified in the `options` config. The `options` config accepts a `validateFile` prop that can be used to replace the default file validation performed when selecting files for upload. The `validateFile` prop must be provided with a function that takes in a single file and return `true` if the file is considered valid. 
...
It is important to ensure that files also adhere to the [S3 limits](https://docs.aws.amazon.com/general/latest/gr/s3.html#limits_s3) when creating custom validation. A file **will** fail during upload if it exceeds the S3 limits, even if the custom validation succeeded when selecting the file.
</Message>
