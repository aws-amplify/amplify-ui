...
At a minimum you must include the `alt` and `path` props. `path` refers to the [Amplify Storage path](TODO add link), which is `string | ((input: { identityId?: string }) => string)`.
...
Note: A new `Storage.get` request is made only when the `path` changes.
