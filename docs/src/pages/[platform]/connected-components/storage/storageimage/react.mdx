import { Accordion, NavLink, MdOutlineAutoAwesome, Alert, Image, ExpanderTitle } from '@aws-amplify/ui-react';
...
<Accordion.Container>
  <Accordion.Item key={"title"} value={"title"}>
    ...
    <Accordion.Content>
      Versions 3.0.1x and earlier use `imgKey`, `accessLevel`, and `identityId` in place of the `path` prop. See [Amplify Gen 1 documentation](https://docs.amplify.aws/gen1/react/build-a-backend/storage/download/) for more information. <br />
      ```diff
      <StorageImage 
         alt="sleepy-cat" 
      -  path="guest/cat.jpg"
...
<Accordion.Container>
  <Accordion.Item key={"title"} value={"title"}>
    ...
    <Accordion.Content>
      Versions 3.0.1x and earlier use `onStorageGetError` in place of the `onGetUrlError` prop. <br />
    ```diff
    <StorageImage
       alt="fallback cat"
       accessLevel="guest"
