...
The `shouldAutoProceed` prop  will automatically start the download after a file is selected. By default is set to `false`.
...
The `multiple` prop allows you to select more then one file for upload. The default is set as `true`. 
...
The `isResumable` prop allows uploads to be resumable. The default is set as `false`.
...
You can override parts of the FileUploder component by passing in a `components` prop. This can include the `UploadDropZone`, `UploadButton`, `Previewer` and the `Tracker` components.
...
The `Preview` component controls the preview page that is shown before an upload begins.
...
The `Tracker` component controls how the files added will look when being displayed inside the preview component.
