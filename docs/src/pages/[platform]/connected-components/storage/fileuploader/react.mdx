...
MaxSizeExample, MultipleExample, ResumableExample, ShowImagesExample, FileUploaderSuccessExample, PreviewerComponentsExample, TrackerComponentsExample, UploadButtonComponentsExample, OnBeforeUploadExample } from './examples'
...
You can use the `onBeforeUpload` to update process files before they are uploaded. With this handlern you can do things like:
* Perform custom validations like making sure a file is valid
* Rename files with unique IDs
* Perform image compression

The `onBeforeUpload` function takes a `FileStatus` object and should return either a `FileStatus` object with any modifications OR a `Promise` that returns a `FileStatus` object on resolve or an error message on reject.

<Example>
  <OnBeforeUploadExample />
  <ExampleCode>
    ```jsx file=./examples/OnBeforeUploadExample.tsx
    ```
  </ExampleCode>
</Example>
