...
## Using Storage with the Authenticator
When using private or protected images, you'll need to wrap the `StorageImage` or `StorageManager` component in the `Authenticator`. This allows the components to infer the Cognito `identityId` of the currently signed-in user.

<Example>
  <Image alt='cat' src='/cats/1.jpg' width="400px" height="400px" />
  <ExampleCode>
    ```jsx file=./storageimage/examples/WithAuthenticator.tsx
    ```
  </ExampleCode>
</Example>
