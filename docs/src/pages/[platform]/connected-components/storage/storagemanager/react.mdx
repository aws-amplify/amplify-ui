...
<Accordion.Container>
  <Accordion.Item key={"title"} value={"title"}>
    ...
    To migrate to a newer version, replace `accessLevel` with `path`. If you were using `path` already, append the `public`, `private` or `protected` to the beginning of the path.
    ```diff
      <StorageManager
    -   accessLevel="guest"
    +   path="public/"
