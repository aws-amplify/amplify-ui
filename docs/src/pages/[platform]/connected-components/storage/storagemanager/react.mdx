...
The `files` state is managed within the StorageManager component. To allow for clearing the internal `files` state, StorageManaer exposes a custom ref handle to the parent component with a `clearFiles()` method.

<Example>
  <StorageManagerFilesRefExample />
  <ExampleCode>
    ```jsx file=./examples/StorageManagerFilesRefExample.tsx
    ```
  </ExampleCode>
</Example>
