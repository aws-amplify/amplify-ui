...
<Accordion.Container>
  <Accordion.Item key={"title"} value={"title"}>
    ...
    To migrate to a later version, replace `accessLevel` with `path`. If you were using `path` already, append the `accessLevel` to the beginning of the path.
    ```diff
      <StorageManager
    -   accessLevel="guest"
    +   path="public/"
    ...
    -   accessLevel="private"
    -   path="images/"
    +   path={({identityId}) => `private/images/${identityId}/`}
        maxFileCount={1}
      />
    ```
    </Accordion.Content>
