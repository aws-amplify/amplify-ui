...
The available StepperField states include `isDisabled` and `isReadonly`. A disabled field will not be focusable not mutable and will not be submitted with form data. A read-only field cannot be edited by the user.
