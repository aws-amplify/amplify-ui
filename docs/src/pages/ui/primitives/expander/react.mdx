...
Expanders enables users to expand or collapse a set of sections.
...
To use the Expander as controlled component, specify the `value` of the item(s) to expand and used in conjunction with `onChange`.
...
You may want to enable the Expanders with type `single` to collapse all items. In this case, set the `isCollapsible` prop to true.
...
To override styling on all Expanders, you can set the Amplify CSS variables with the built-in `.amplify-expander`, `.amplify-expander__item`, `.amplify-expander__header`, `.amplify-expander__trigger`, `.amplify-expander__icon`, `.amplify-expander__content` and `.amplify-expander__content__text` class.
