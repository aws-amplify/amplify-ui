...
<Alert variation="success" title="This is the title"> ^ This Alert has a title</Alert>
```
...
Use the `withIcon` prop to change whether the Alert includes an icon. Defaults to `true` (includes icon).

```jsx
<Alert variation="info">This Alert shows an icon by default</Alert>
<Alert variation="error" withIcon={false}>This Alert does not have an icon</Alert>
```

<Example>
  <Flex direction="column" gap="1rem">
    <Alert variation="info">{`<==`} This Alert shows an icon by default</Alert>
    <Alert variation="error" withIcon={false}>
