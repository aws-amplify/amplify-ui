...
Use the `size` prop to change the icon size. Available options are `small` and `large` (in addition to the default size).
...
Use the `fill` prop to change the icon color. By default the icon color is set to [`currentColor`](https://developer.mozilla.org/en-US/docs/Web/CSS/color_value#currentcolor_keyword)(inherits color from current font color).
