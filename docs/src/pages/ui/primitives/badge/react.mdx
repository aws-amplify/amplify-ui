import { Badge } from '@aws-amplify/ui-react';
import { BadgeDemo } from './demo';
import { Example } from '@/components/Example';
import Block from 'amplify-docs/src/components/Block';
import BlockSwitcher from 'amplify-docs/src/components/BlockSwitcher';

A Badge is a small visual element to denote a status or message about an item. A small, color-coded visual element that contains letters or numbers, that you can use to label, categorize, or organize items.

## Installation

<BlockSwitcher>
<Block name="npm">

```shell
npm install @aws-amplify/ui-react
```

</Block>
<Block name="yarn">

```shell
yarn add @aws-amplify/ui-react
```

</Block>
</BlockSwitcher>

## Demo

<BadgeDemo />

## Usage

Import the Badge component and styles.

```jsx
import { Badge } from '@aws-amplify/ui-react';
import '@aws-amplify/ui-react/styles.css';

<Badge>Default badge</Badge>;
```

<Example>
  <Badge>Default badge</Badge>
</Example>

### Variations

Use the `variation` prop to change the Badge variation. In addition to the default, available options are `info`, `error`, `warning` and `success`.

```jsx
<Badge>Default</Badge>
<Badge variation="info">Info</Badge>
<Badge variation="error">Error</Badge>
<Badge variation="warning">Warning</Badge>
<Badge variation="success">Success</Badge>
```

<Example>
  <Badge>Default</Badge>
  <Badge variation="info">Info</Badge>
  <Badge variation="error">Error</Badge>
  <Badge variation="warning">Warning</Badge>
  <Badge variation="success">Success</Badge>
</Example>

### Sizes

Use the `size` prop to change the Badge size. In addition to the default, available options are `small` and `large`.

```jsx
<Badge size="small">Small</Badge>
<Badge>Default</Badge>
<Badge size="large">Large</Badge>
```

<Example>
  <Badge size="small">Small</Badge>
  <Badge>Default</Badge>
  <Badge size="large">Large</Badge>
</Example>

## CSS Styling

### Global styling

To override styling on all Badges, you can set the Amplify CSS variables or use the built-in `.amplify-badge` class.

```css
/* styles.css */
:root {
  --amplify-components-badge-background-color: yellow;
}
/* OR */
.amplify-badge {
  background-color: yellow;
}
```

To replace the Badge styling, unset it:

```css
.amplify-badge {
  all: unset;
  /* Add your styling here*/
}
```

### Local styling

To override styling on a specific Badge, you can use (in order of increasing specificity): a class selector, data attributes, or style props.

_Using a class selector:_

```css
/* styles.css */
.flagged {
  color: white;
  background-color: crimson;
  border-radius: 3px;
}
```

```jsx
import './styles.css';

<Badge className="flagged">13</Badge>;
```

<Example>
  <Badge className="flagged">13</Badge>
</Example>

_Using data attributes:_

```css
/* styles.css */
/* Override only info variation styles */
.amplify-badge[data-variation='info'] {
  background-color: rebeccapurple;
}

/* Override only large size styles */
.amplify-badge[data-size='large'] {
  border: 1px solid black;
}
```

```jsx
import './styles.css';

<Badge variation="info">Purple background</Badge>
<Badge size="large">Black border</Badge>
```

_Using style props:_

```jsx
<Badge style={{ backgroundColor: 'green', color: 'white' }}>Green</Badge>;
{
  /* OR */
}
<Badge backgroundColor="purple" color="white">
  Purple
</Badge>;
```

<Example>
  <Badge style={{ backgroundColor: 'green' }}>Green</Badge>
  <Badge backgroundColor="purple">Purple</Badge>
</Example>
