...
The PasswordField primitive can be used allow users to input passwords.
...
#### How to choose an `autoComplete` option

If customers are logging in, you don't need to do set `autoComplete` because `current-password` is already the default.
If customers are signing up for a new account, set `autoComplete` to `new-password`.
...
The PasswordField primitive is accessible by default. A matching `label` HTML element is rendered with the `PasswordField` by default,
...
Flex styling props will be applied to the PasswordField wrapping Flex component, whereas other style props will be applied to the field. This allows us to change the layout of the label and field, while also styling the input field.
