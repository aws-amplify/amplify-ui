...
<Expander type="multiple" defaultValue={['TableProps']}>
  
<ExpanderItem title="TableProps" value="TableProps">
    <ReactMarkdown
      remarkPlugins={[remarkGfm]}
      children={`
| name | type | default | description |
| ---- | ---- | ------- | ----------- |
| caption | ReactNode | - | Optional caption which serves as an accessible title/caption for the table. |
| highlightOnHover | boolean | false | If set to true, will highlight a table row on hover. Default is false. |
| size | Sizes | - | Affects font size and padding. Default is in between 'small' and 'large'. |
| variation | TableVariation | - | Sets a supported style variation. Default is no specific variation. |
