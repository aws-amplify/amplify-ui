...
The `<option>` tags nested inside the component define the available options in the drop-down list.
...
Use the `hasError` and `errorMessage` props to mark a SelectField as having an validation error.
