...
import { ControlledSliderField, DaySlider, SliderInForm } from './OLD_examples';
...
To control the range of the SliderField, use the `min` and `max` props. To control the interval between selectable values, use the `step` prop (defaults to 1).
...
The `Slider` will render an input for the thumb when used within a form to ensure that value will be submitted with form data.
