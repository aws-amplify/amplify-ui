...
<Fragment>
  {({ platform }) => import(`./advanced/conditional-rendering.${platform}.mdx`)}
</Fragment>
...
Here's an example that uses above methods to conditionally render different components based on current authentication status. Note that this can be extended to support routed application.
