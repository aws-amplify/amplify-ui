...
| Class                           | Description                                                                                             |
| ------------------------------- | ------------------------------------------------------------------------------------------------------- |
| `amplify-rating`                | Top level `<Flex>` component that wraps all rating elements                                             |
| `amplify-rating-icon-container` | Class applied to the `<span>` that wraps each rating icon                                               |
| `amplify-rating-label`          | Class applied to the `<label>` that wraps each rating icon                                              |
| `amplify-rating-icon`           | Class applied alongside both the filled and empty icon classes in a mixed(partially filled) rating icon |
| `amplify-rating-icon-filled`    | Class applied to the `<span>` that wraps a filled rating icon                                           |
| `amplify-rating-icon-empty`     | Class applied to the `<span>` that wraps an empty rating icon                                           |
