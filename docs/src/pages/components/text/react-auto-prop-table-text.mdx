...
<Expander type="multiple" defaultValue={['TextProps']}>
...
| ---- | ---- | ------- | ----------- |
| variation | TextVariation | - | This should be the primary way to handle different styles of text. Lower-level text styling attributes like color can be set directly, that should be more of an escape hatch. |
| isTruncated | boolean | - | This attribute will be used to indicate if the text component should truncate text that exceeds the width of the text element.  Truncated text will render an ellipsis. |
