...
As of v2.15.0 of `@aws-amplify/ui-react` which included the release of Geo components, users of the Jest testing
framework may run into the following error when attempting to run tests:
...
This is a known problem when using the `jsdom` library (a dependency of Jest) with a package that uses the unrecognized
function. See [this issue](https://github.com/jsdom/jsdom/issues/1721).
