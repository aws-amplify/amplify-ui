...
async function createAllPropsTables() {
  for await (const componentFilepath of globbyStream(
    ...
    const { MDN, htmlElement } = getFrontmatter(
      fs.readFileSync(componentFilepath, 'utf8')
    );
    ...
    fs.writeFileSync(
      ...
      Output(componentName, tableAndExpanders, MDN, htmlElement)
    );
...
function Output(displayName, tableAndExpanders, MDN, htmlElement) {
  return `
