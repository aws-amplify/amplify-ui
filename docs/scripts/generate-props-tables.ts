...
async function createAllPropsLists() {
  for await (const componentFilepath of globbyStream(
    ...
    const propsLists = [];
    const mainPropsList = createComponentPropsObject(catalog, componentName);
    ...
    if (componentName in componentsWithChildren) {
      componentsWithChildren[componentName].forEach((childName) => {
        propsLists.push(createComponentPropsObject(catalog, childName));
      });
    }
 ...
 * @param componentName
 * @returns propList object that includes a name, and an array of props
 */
 ...
 * @param catalog
 * @param componentName
 * @returns a componentPropObject which includes the component name,
 * its main prop list, and any utility (layout) prop lists.
 */
function createComponentPropsObject(
  catalog: Catalog,
  componentName: ComponentName
) {
  ...
  const componentPropObject = {
    name: componentName,
    props: mainProps,
    utilityProps: utilityProps,
  };

  return componentPropObject;
}
