...
echo -e "${BLUE_BOLD}Sleep for $TIME_TO_WAIT seconds...${COLOR_END}"
sleep $TIME_TO_WAIT

# Check if the log file only has one line
if [[ $(wc -l <"$LOG_FILE") -eq 1 && $(head -n 1 "$LOG_FILE") == "info Starting logkitty" ]]; then
  echo -e "${RED_BOLD}Failed to get the logging messages. Please increase TIME_TO_WAIT.${COLOR_END}"
  exit 1
fi
...
if [[ $(wc -l <"$LOG_FILE") -eq 1 ]]; then
  for start_message in "${START_MESSAGES[@]}"; do
    if echo $(head -n 1 "$LOG_FILE") | grep -q "$start_message"; then
      echo -e "${RED_BOLD}Failed to get the logging messages. Please increase TIME_TO_WAIT.${COLOR_END}"
      echo -e "${BLUE_BOLD}Full log:${COLOR_END}"
      cat "$LOG_FILE"
      exit 1
    fi
  done
...
echo -e "${BLUE_BOLD}Checking log file \"$LOG_FILE\" for errors...${COLOR_END}"
HAS_ERROR=false
...
while read -r line; do
  ...
  # Check if there's any line has "NN:NN:NN"(N is 0-9) and "Error" or "ERROR".
    ...
    echo -e "${RED_BOLD}Connection error:${COLOR_END}"
    echo -e $line
    LINE_ERROR=true
  fi
  for exception in "${EXCEPTIONS[@]}"; do
    if [[ "$line" == *"$exception"* ]]; then
      echo -e "${YELLOW_BOLD}Exception found:${COLOR_END}"
      echo -e $line
      LINE_ERROR=false
      break
    fi
...
if [[ $HAS_ERROR == true ]]; then
  echo -e "${RED_BOLD}Errors found in log file \"$LOG_FILE\":${COLOR_END}"
  echo -e "${BLUE_BOLD}Full log:${COLOR_END}"
  cat "$LOG_FILE"
  exit 1
elif [ ! -s "$LOG_FILE" ]; then
  echo -e "${RED_BOLD}log file \"$LOG_FILE\" is empty:${COLOR_END}"
  echo -e "${BLUE_BOLD}Full log:${COLOR_END}"
  cat "$LOG_FILE"
  exit 1
fi
...
echo -e "${BLUE_BOLD}Full log:${COLOR_END}"
cat "$LOG_FILE"
echo -e "${GREEN_BOLD}No errors found in log file \"$LOG_FILE\". Exiting successfully.${COLOR_END}"
exit 0
