...
touch $LOG_FILE
echo -e "${BLUE_BOLD}Installing react-native-log-ios...${COLOR_END}"
npm install -D react-native-log-ios
echo -e "${BLUE_BOLD}Logging errors from mega-app $MEGA_APP_NAME to $LOG_FILE in the background...${COLOR_END}"
...
echo -e "${BLUE_BOLD}npm start${COLOR_END}"
npm start &

# Step 2: Run npm run android in the background
echo -e "${BLUE_BOLD}npx pod-install${COLOR_END}" # To prevent "AsyncStorage is null." https://react-native-async-storage.github.io/async-storage/docs/install/
npx pod-install
echo -e "${BLUE_BOLD}Running npm run ios...${COLOR_END}"
npm run ios
