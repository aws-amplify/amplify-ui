...
type MockActionTaskStatus = Exclude<TaskStatus, 'OVERWRITE_PREVENTED'>;

type MockActionTasks = {
  [K in MockActionTaskStatus]: Omit<ActionTask, 'status'> & {
    status: K;
  };
};
...
export const MOCK_DELETE_TASKS: MockActionTasks = {
  CANCELED: {
    data: {
      ...MOCK_DELETE_DATA,
      id: 'CANCELED-ID',
