...
export interface UsePaginateInput<T> {
  items?: T[];
...
export const usePaginate = <T>({
  ...
  const visttedRef = React.useRef(page);

  const handleReset = React.useRef(() => {
    setCurrentPage(page);
    visttedRef.current = page;
  }).current;

  return React.useMemo((): UsePaginateState<T> => {
    const hasItems = Array.isArray(items);

    const highestPageVisited = visttedRef.current;
    ...
    return {
      ...
      handlePaginate: (page) => {
        ...
        if (page > currentPage) visttedRef.current = page;
