...
var F = (t, e, r) =>
    e in t
      ...
      : (t[e] = r),
  p = (t, e) => {
    for (var r in e || (e = {})) _.call(e, r) && F(t, r, e[r]);
    if (w) for (var r of w(e)) T.call(e, r) && F(t, r, e[r]);
    return t;
  },
  f = (t, e) => Q(t, Z(e));
var k = (t, e) => {
  var r = {};
  for (var o in t) _.call(t, o) && e.indexOf(o) < 0 && (r[o] = t[o]);
  if (t != null && w)
    for (var o of w(t)) e.indexOf(o) < 0 && T.call(t, o) && (r[o] = t[o]);
  return r;
};
import {
  assign as g,
...
import { Auth as ie, Amplify as se } from 'aws-amplify';
import { createMachine as Y, assign as i, sendUpdate as s } from 'xstate';
import { get as I, isEmpty as M } from 'lodash';
var d;
(function (n) {
  (n.SMS_MFA = 'SMS_MFA'),
    (n.SOFTWARE_TOKEN_MFA = 'SOFTWARE_TOKEN_MFA'),
    (n.NEW_PASSWORD_REQUIRED = 'NEW_PASSWORD_REQUIRED'),
    (n.RESET_REQUIRED = 'RESET_REQUIRED'),
    (n.MFA_SETUP = 'MFA_SETUP');
})(d || (d = {}));
var v = ['username', 'email', 'phone_number'],
  le = ['amazon', 'google', 'facebook'];
import { Auth as m } from 'aws-amplify';
var D = Y(
  {
    initial: 'init',
    id: 'signInActor',
  ...
  },
  {
    actions: {
      handleInput: i({
        formValues(t, e) {
          let { name: r, value: o } = e.data;
          return f(p({}, t.formValues), { [r]: o });
        },
      }),
      setUser: i({ user: (t, e) => e.data.user || e.data }),
      setUsername: i({
      ...
      setRemoteError: i({
        remoteError: (t, e) => {
          var r;
          return ((r = e.data) == null ? void 0 : r.message) || e.data;
        },
      }),
      setChallengeName: i({
        challengeName: (t, e) => {
          var r;
          return (r = e.data) == null ? void 0 : r.challengeName;
        },
      }),
      setConfirmSignUpIntent: i({ redirectIntent: 'confirmSignUp' }),
      setConfirmResetPasswordIntent: i({
        redirectIntent: 'confirmPasswordReset',
      }),
      setUnverifiedAttributes: i({
        unverifiedAttributes: (t, e) => e.data.unverified,
      }),
      clearChallengeName: i({ challengeName: void 0 }),
      clearError: i({ remoteError: '' }),
      clearFormValues: i({ formValues: {} }),
      clearUnverifiedAttributes: i({ unverifiedAttributes: void 0 }),
      clearAttributeToVerify: i({ attributeToVerify: void 0 }),
    },
    guards: {
      shouldConfirmSignIn: (t, e) => {
        let r = I(e, 'data.challengeName');
        return [d.SMS_MFA, d.SOFTWARE_TOKEN_MFA].includes(r);
      },
      shouldRedirectToConfirmSignUp: (t, e) =>
        e.data.code === 'UserNotConfirmedException',
      shouldRedirectToConfirmResetPassword: (t, e) =>
        e.data.code === 'PasswordResetRequiredException',
      shouldSetupTOTP: (t, e) => I(e, 'data.challengeName') === d.MFA_SETUP,
      shouldForceChangePassword: (t, e) =>
        I(e, 'data.challengeName') === d.NEW_PASSWORD_REQUIRED,
      shouldAutoSignIn: (t) => !!(t.intent && t.intent === 'autoSignIn'),
      shouldRequestVerification: (t, e) => {
        let { unverified: r, verified: o } = e.data;
        return M(o) && !M(r);
      },
    },
    services: {
      ...
      async confirmSignIn(t, e) {
        let { challengeName: r, user: o } = t,
          { confirmation_code: a } = e.data,
          n;
        return (
          (r === d.SMS_MFA || r === d.SOFTWARE_TOKEN_MFA) && (n = r),
          m.confirmSignIn(o, a, n)
        );
      },
      async forceNewPassword(t, e) {
        let { user: r } = t,
          o = I(e, 'data.password');
        return m.completeNewPassword(r, o);
      },
      async verifyTotpToken(t, e) {
        let { user: r } = t,
          { confirmation_code: o } = e.data;
        return m.verifyTotpToken(r, o);
      },
      async federatedSignIn(t, e) {
        let { provider: r } = e.data;
        return await m.federatedSignIn({ provider: r });
      },
      async checkVerifiedContact(t, e) {
        let { user: r } = t;
        return await m.verifiedContact(r);
      },
      async verifyUser(t, e) {
        let r = await m.verifyCurrentUserAttribute(e.data.unverifiedAttr);
        return (t.attributeToVerify = e.data.unverifiedAttr), r;
      },
      async confirmVerifyUser(t, e) {
        let { attributeToVerify: r } = t,
          { confirmation_code: o } = e.data;
        return await m.verifyCurrentUserAttributeSubmit(r, o);
      },
    },
  }
);
import { createMachine as X, assign as l, sendUpdate as u } from 'xstate';
import $ from 'lodash/isEmpty';
import J from 'lodash/merge';
var O = (t) => {
  let { password: e, confirm_password: r } = t;
  if (!e && !r) return null;
  if (!e || !r || e !== r)
    return { confirm_password: 'Your passwords must match' };
};
var z = async (t, e) => {
  let r = await Promise.all(e.map((a) => a(t))),
    o = J({}, ...r);
  return $(o) ? Promise.resolve() : Promise.reject(o);
};
import { Auth as U } from 'aws-amplify';
var G = X(
  {
    id: 'signUpActor',
    initial: 'init',
    states: {
          ...
          validation: {
            ...
            states: {
              pending: {
                invoke: {
                  src: 'validateFields',
                  onDone: { target: 'valid', actions: 'clearValidationError' },
      ...
      resolved: {
        type: 'final',
        data: (t) => {
          let { username: e, password: r } = t.authAttributes,
            o = !!(e && r);
          return {
            user: t.user,
            intent: o ? 'autoSignIn' : null,
            authAttributes: { username: e, password: r },
          };
        },
      },
    },
  ...
  {
    guards: {
      shouldInitConfirmSignUp: (t) => t.intent && t.intent === 'confirmSignUp',
    },
    actions: {
      setUser: l({
        user: (t, e) => {
          var r;
          return (r = e.data.user) != null ? r : e.data;
        },
      }),
      setRemoteError: l({
        remoteError: (t, e) => {
      ...
      setFieldErrors: l({ validationError: (t, e) => e.data }),
      setCredentials: l({
        authAttributes: (t) => {
          var a, n;
          let [e] = (a = t.login_mechanisms) != null ? a : ['username'],
            r = t.formValues[e],
            o = (n = t.formValues) == null ? void 0 : n.password;
          return { username: r, password: o };
        },
      }),
      handleInput: l({
        formValues: (t, e) => {
      ...
      clearError: l({ remoteError: '' }),
      clearFormValues: l({ formValues: {} }),
      clearValidationError: l({ validationError: {} }),
    },
    services: {
      async confirmSignUp(t, e) {
        let { username: r, confirmation_code: o } = e.data;
        return U.confirmSignUp(r, o);
      },
      async resendConfirmationCode(t, e) {
        let { username: r } = e.data;
        return U.resendSignUp(r);
      },
      async federatedSignIn(t, e) {
        let { provider: r } = e.data;
        return await U.federatedSignIn({ provider: r });
      },
      async signUp(t, e) {
        let b = t,
          { formValues: y } = b,
          ...
          { password: r } = h,
          o = k(h, ['password']),
          { login_mechanisms: a } = b,
          [n] = a != null ? a : ['username'];
        o.phone_number &&
          (o.phone_number = o.phone_number.replace(/[^A-Z0-9+]/gi, ''));
        let c = o[n];
        return (
          delete o[n],
          delete o.confirm_password,
          await U.signUp({ username: c, password: r, attributes: o })
        );
      },
...
import { createMachine as ee } from 'xstate';
import { Auth as te } from 'aws-amplify';
var B = ee(
  {
    initial: 'pending',
    id: 'signOutActor',
  ...
  { services: { signOut: () => te.signOut() } }
);
import { createMachine as re, assign as A, sendUpdate as C } from 'xstate';
import { Auth as H } from 'aws-amplify';
var W = re(
  {
    ...
    states: {
      ...
      resetPassword: {
        ...
        states: {
          ...
          submit: {
            entry: [C(), 'setUsername', 'clearError'],
            invoke: {
              src: 'resetPassword',
  ...
  },
  {
    actions: {
      ...
      setUsername: A({ username: (t) => t.formValues.username }),
      handleInput: A({
        formValues: (t, e) => {
          let { name: r, value: o } = e.data;
          return f(p({}, t.formValues), { [r]: o });
      ...
      clearFormValues: A({ formValues: {} }),
      clearError: A({ remoteError: '' }),
      clearUsername: A({ username: void 0 }),
    },
    guards: {
      shouldAutoConfirmReset: (t, e) =>
        !!(t.intent && t.intent === 'confirmPasswordReset'),
    },
    services: {
      async resetPassword(t) {
        var r, o;
        let e =
          (o = (r = t.formValues) == null ? void 0 : r.username) != null
            ? o
            : t.username;
        return H.forgotPassword(e);
      },
      async confirmResetPassword(t) {
        let { username: e } = t,
          { confirmation_code: r, password: o } = t.formValues;
        return H.forgotPasswordSubmit(e, r, o);
      },
    },
  }
);
import { stop as oe } from 'xstate/lib/actions';
var E = (t) => oe(t);
var j = ne(
  {
    id: 'auth',
    ...
    states: {
      ...
      signIn: {
        ...
        on: {
          ...
          'done.invoke.signInActor': [
            ...
            { target: 'resetPassword', cond: 'shouldRedirectToResetPassword' },
      ...
      signUp: {
        ...
        on: {
          SIGN_IN: 'signIn',
          'done.invoke.signUpActor': { target: 'signIn', actions: 'setUser' },
        },
      },
      resetPassword: {
        entry: 'spawnResetPasswordActor',
        exit: E('resetPasswordActor'),
        on: { SIGN_IN: 'signIn', 'done.invoke.resetPasswordActor': 'signIn' },
      },
      signOut: {
        entry: 'spawnSignOutActor',
        exit: [E('signOutActor'), 'clearUser'],
  ...
  {
    actions: {
      forwardToActor: ae((t) => t.actorRef),
      setUser: g({ user: (t, e) => e.data.user || e.data }),
      clearUser: g({ user: void 0 }),
      setAuthConfig: g({
        config(t, e) {
          return e.data.auth;
        },
      ...
      spawnSignInActor: g({
        actorRef: (t, e) => {
          var o, a, n;
          let r = D.withContext({
            authAttributes: (o = e.data) == null ? void 0 : o.authAttributes,
            user: (a = e.data) == null ? void 0 : a.user,
            intent: (n = e.data) == null ? void 0 : n.intent,
            formValues: {},
            validationError: {},
          });
          return x(r, { name: 'signInActor' });
        },
      }),
      spawnSignUpActor: g({
        actorRef: (t, e) => {
          ...
          let r = G.withContext({
            authAttributes:
              (a = (o = e.data) == null ? void 0 : o.authAttributes) != null
                ? a
                : {},
            intent: (n = e.data) == null ? void 0 : n.intent,
            formValues: {},
            validationError: {},
            login_mechanisms:
              (c = t.config) == null ? void 0 : c.login_mechanisms,
          });
          return x(r, { name: 'signUpActor' });
        },
      }),
      spawnResetPasswordActor: g({
        actorRef: (t, e) => {
          var o, a, n;
          let r = W.withContext({
            formValues: {},
            intent: (o = e.data) == null ? void 0 : o.intent,
            username:
              (n = (a = e.data) == null ? void 0 : a.authAttributes) == null
                ? void 0
                : n.username,
            validationError: {},
          });
          return x(r, { name: 'resetPasswordActor' });
        },
      }),
      spawnSignOutActor: g({
        actorRef: (t) => {
          let e = B.withContext({ user: t.user });
          return x(e, { name: 'signOutActor' });
        },
      }),
    },
    guards: {
      shouldRedirectToSignUp: (t, e) => {
        var r;
        return ((r = e.data) == null ? void 0 : r.intent)
          ? e.data.intent === 'confirmSignUp'
          : !1;
      },
      shouldRedirectToResetPassword: (t, e) => {
        var r;
        return ((r = e.data) == null ? void 0 : r.intent)
          ? e.data.intent === 'confirmPasswordReset'
          : !1;
      },
    },
    services: {
      async getCurrentUser() {
        return ie.currentAuthenticatedUser();
      },
...
import { interpret as ce } from 'xstate';
var rt = ce(j, { devTools: !0 }).start();
import { includes as L } from 'lodash';
var P = {
    username: {
      label: 'Username',
      type: 'text',
...
var ut = (t) => {
    var n, c, S, b, y, h;
    let e =
        (c = (n = t.config) == null ? void 0 : n.login_mechanisms) != null
          ? c
          : ['username'],
      r =
        (b = (S = t.actorRef) == null ? void 0 : S.context) == null
          ? void 0
          : b.validationError.username,
      o = 'text',
      a = e
        .filter((R) => L(v, R))
        .map((R) => {
          var N, V;
          return (V = (N = P[R]) == null ? void 0 : N.label) != null
            ? V
            : P.username.label;
        })
        .join(' or ');
    return (
      e.length === 1 &&
        (o =
          (h = (y = P[e[0]]) == null ? void 0 : y.type) != null ? h : 'text'),
      { label: a, type: o, error: r }
    );
  },
  mt = (t) => {
    var n, c;
    let r = (
      ...
      [o, ...a] = r;
    return { primaryAlias: o, secondaryAliases: a };
  },
  de = (t) => {
    var e;
    return (e = t.context.actorRef) == null ? void 0 : e.getSnapshot();
  },
  lt = (t) => {
    var e;
    return (e = de(t)) == null ? void 0 : e.context;
  };
var At = {
  de: {
...
export {
  d as AuthChallengeNames,
  q as FederatedIdentityProviders,
  P as authInputAttributes,
  j as authMachine,
  rt as authService,
  At as dict,
  lt as getActorContext,
  de as getActorState,
  ut as getAliasInfoFromContext,
  mt as getConfiguredAliases,
  le as socialProviderLoginMechanisms,
  v as userNameAliasArray,
};
