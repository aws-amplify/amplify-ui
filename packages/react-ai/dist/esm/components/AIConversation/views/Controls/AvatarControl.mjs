...
const AvatarControl = () => {
    ...
    const defaultIcon = role === 'assistant' ? React.createElement(DEFAULT_AI_ICON, null) : React.createElement(DEFAULT_USER_ICON, null);
    const defaultDisplayName = role === 'user' ? 'User' : 'Assistant';
    return (React.createElement(Container, { "data-testid": 'avatar' },
        React.createElement(AvatarIcon, { "data-testid": `avatar-icon-${role}` }, avatar?.avatar ?? defaultIcon),
        React.createElement(AvatarDisplayName, null, avatar?.username ?? defaultDisplayName)));
};
