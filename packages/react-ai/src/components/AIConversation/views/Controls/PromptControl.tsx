...
import { AIConversationElements, SuggestedPromptsContext } from '../../context';
    ...
    return (
      <ButtonGroupBase {...props} ref={ref}>
        {suggestedPromptsArray.map((prompt, index) => {
          return (
            <PromptCard key={index} onClick={() => console.log('clicked me')}>
              <Text className={classNames(`${PROMPT_CARD}__header`, `${PROMPT_CARD}__text`)}>{prompt.header}</Text>
...
> {
  ({ textInputRef }: { textInputRef: React.Ref<HTMLInputElement | undefined> }): React.JSX.Element;
  Container: T['View'];
}
