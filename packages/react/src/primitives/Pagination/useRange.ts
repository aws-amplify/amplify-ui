...
export const useRange = (
  ...
  const range = useMemo(() => {
    ...
    const shouldRenderLeftEllipsis = leftSiblingPage > 2;
    const shouldRenderRightEllipsis = rightSiblingPage < lastPage - 1;

    /**
     * Case 2: Only render ellipsis dots on the left side (e.g. [1, '...', 7, 8, 9, 10]).
     */
    if (shouldRenderLeftEllipsis && !shouldRenderRightEllipsis) {
