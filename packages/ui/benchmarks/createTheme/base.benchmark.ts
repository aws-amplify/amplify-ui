...
import { createTheme, defineComponentTheme } from '../../src';
...
bench('createTheme (with component theme)', () => {
  const theme = createTheme({
    name: 'my-theme',
  ...
  .mean([6, 'ms'])
  .types([35000, 'instantiations']);

bench('createTheme (with custom tokens)', () => {
  const theme = createTheme({
    name: 'my-theme',
    primaryColor: 'pink',
