...
const shouldManualSignIn = (context: AuthActorContext, { data }: AuthEvent) => {
  const signUpOutput = data as SignUpOutput;
  const nextStep = signUpOutput?.nextStep.signUpStep;
  const contextStep = context.step;

  const result = nextStep === 'DONE' && contextStep === 'CONFIRM_SIGN_UP';
